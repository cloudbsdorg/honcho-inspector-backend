...
# Mode/repo compatibility. The backend repo owns the Spring Boot
# jar; it produces backend-only + all-in-one. The UI repo owns the
# Angular UI; it produces ui-only. The script writes either to the
# honcho-inspector-backend repo or the honcho-inspector-ui repo
# depending on REPO_NAME.
case "$REPO_NAME:$MODE" in
    honcho-inspector-backend:ui-only) printf "%s + %s is built by the honcho-inspector-ui repo, not this one\n" "$REPO_NAME" "$MODE" >&2; exit 2 ;;
    honcho-inspector-ui:backend-only|honcho-inspector-ui:all-in-one) printf "%s + %s is built by the honcho-inspector-backend repo, not this one\n" "$REPO_NAME" "$MODE" >&2; exit 2 ;;
esac
case "$CMD" in
...
case "$MODE" in
    ui-only)
        # The standalone UI image is built in the ui repo, not here.
        # This branch is unreachable in the backend repo because the
        # case "$REPO_NAME:$MODE" guard above rejects the combination.
